...
def init_base_vel_cmd(num_envs):
    ...
    base_vel_cmd_input = torch.zeros((num_envs, 3), dtype=torch.float32) # x,z,yaw

# Modify base_vel_cmd to use the tensor directly
def base_vel_cmd(env: DirectRLEnv) -> torch.Tensor:
    global base_vel_cmd_input
...
def get_envs(cfg):
    env = gym.make("Isaac-Dronep-Play-Direct-v0", cfg=cfg)
